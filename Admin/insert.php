...
if(!empty($_POST)) {
    ...
    $image              = checkInput($_FILES["image"]["name"]);;//tableau des tableaux
    $imagePath          = '../images/'. basename($image); //nom de base
    ...
    if(empty($image)) {
        ...
        if($isUploadSuccess) {
            if(!move_uploaded_file($_FILES["image"]["tmp_name"], $imagePath)) {// le nom tmp__name est obligatoire , il designe l'emplacement temporaire;
